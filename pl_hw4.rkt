...
(define (eval expr)
  (type-case PWAE expr
    ...
    [poohadd (l) (foldl (lambda (x s) (bin-op + s (eval x))) (eval (first l)) (rest l))]
    [poohsub (l) (foldl (lambda (x s) (bin-op - s (eval x))) (eval (first l)) (rest l))]))
...
(test (run "{with {x {pooh 3 2 {0 4} -}} {+ x x}}") '(2 -2 -2 -6))
(test (run "{3 2}") '(3 2))
(test (run "{with {x 3}{with {x 4} {+ x 4}}}") '(8))
(test (run "{+ 3 {5 1 0}}") '(8 4 3))